...
async def login(login_data: LoginRequest):
    ...
    with engine.connect() as conn:
        ...
        result = conn.execute(
            text(
                "SELECT username, password_hash, role, full_name, email FROM pg_auto_tool_table_users WHERE email = :email"),
...
async def signup(user_data: UserCreate):
    ...
    with engine.begin() as conn:
        # Check if user exists using new table prefix
        existing = conn.execute(
            text("SELECT id FROM pg_auto_tool_table_users WHERE username = :username"),
        ...
        conn.execute(
            text("""
                INSERT INTO pg_auto_tool_table_users 
...
async def list_users():
    engine = get_engine()
    with engine.connect() as conn:
        # Fetch all users using mappings for dictionary-style access
        result = conn.execute(
            text("SELECT id, username, full_name, email, is_active, is_admin, role FROM pg_auto_tool_table_users")
        ).mappings().fetchall()
...
async def update_user(user_id: int, user_data: UserUpdate):
    ...
    engine = get_engine()
    with engine.begin() as conn:
        query = text(f"UPDATE pg_auto_tool_table_users SET {', '.join(update_fields)} WHERE id = :user_id")
...
async def delete_user(user_id: int, current_user: dict = Depends(get_current_user)):
    ...
    with engine.begin() as conn:
        result = conn.execute(
            text("DELETE FROM pg_auto_tool_table_users WHERE id = :user_id"),
            {"user_id": user_id}
        )
        if result.rowcount == 0:
            raise HTTPException(status_code=404, detail="User not found")
